...
from models import testCIFAR10CNN
...
from fuzzy_detectior.CIFAR10.src.shap_signature import generate_shap_signatures_dens5
...
def prepare_detector_data(model, clean_data, adv_data, attack_type, device):
    ...
    clean_signatures = generate_shap_signatures_dens5(model, clean_images, device)

    print(f"[{attack_type}] Generating noisy clean SHAP signatures...")
    clean_signatures_noisy = generate_shap_signatures_dens5(model, clean_images_noisy, device)

    print(f"[{attack_type}] Generating adversarial SHAP signatures...")
    adv_signatures = generate_shap_signatures_dens5(model, adv_images, device)
