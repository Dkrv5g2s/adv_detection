...
def rs_fgsm_attack(model, x, y, epsilon=8 / 255, alpha=None):
    ...
    Algorithm 1: FGSM adversarial training with random start
...
def n_fgsm_attack(model, x, y, epsilon=8 / 255, alpha=None, k=None):
    """
    N-FGSM: Noise-FGSM (stronger noise, no intermediate clipping)

    Reference: de Jorge et al. 2022 "Make Some Noise"
    Key insight: Use larger initial noise (k > ε) and avoid clipping
    ...
    Args:
        ...
        k: Initial noise magnitude (default: 2ε as per paper)

    Returns:
        x_adv: Adversarial examples
    """
    if alpha is None:
        alpha = epsilon  # Paper uses α = ε (not ε + 2/255)

    if k is None:
        k = 2 * epsilon  # Paper recommendation: k = 2ε

    # Step 1: Sample larger noise δ ~ U(-k, k) where k > ε
    delta = torch.empty_like(x).uniform_(-k, k)
    delta.requires_grad = True

    # Step 2: Compute gradient on x + δ (NO clipping before gradient)
    output = model(x + delta)
    loss = F.cross_entropy(output, y)
    loss.backward()
...
def train_n_der(model, train_loader, optimizer, scheduler, epoch, device,
                epsilon=8 / 255, beta=0.1, gamma=0.0, k=None):
    """
    N-DER Training

    Args:
        k: Noise magnitude for N-FGSM (預設 2ε)
    """
    if k is None:
        k = epsilon * 2
