...
from fuzzy_detectior.MNIST.src.fuzzy_detector import train_fuzzy_detector , \
    test_fuzzy_detector
...
def main():
    ...
    print("=== Cross-Dataset Fuzzy Adversarial Detection (Optimized) ===")
    print(f"Training: MNIST + {TRAIN_ATTACK_TYPES} (Combined)")
    print(f"Testing: CIFAR-10 + {TEST_ATTACK_TYPES}")
    print(f"Training samples: {TRAIN_SAMPLES}, Test samples: {TEST_SAMPLES}")
    print(f"Method: {CLASS_REFERENCE_SAMPLES}-sample class-based SHAP differences → Unified Fuzzy Detector")

    # === 第一階段：在 MNIST 上訓練偵測器 ===
    print("\n=== Phase 1: Training unified fuzzy detector on MNIST ===")
    ...
    # 準備合併的訓練資料
    all_clean_features_diff = []
    all_adv_features_diff = []

    print("\n--- Preparing combined training data for fuzzy detector ---")
    for attack_type in TRAIN_ATTACK_TYPES:
        if attack_type in train_results:
            ...
            # 準備 MNIST 訓練資料 (使用優化的同類別參考方法)
            clean_features_diff, adv_features_diff = prepare_detector_data(
                mnist_model, clean_data, adv_data, attack_type, device
            )

            all_clean_features_diff.append(clean_features_diff)
            all_adv_features_diff.append(adv_features_diff)

    # 合併所有攻擊類型的資料
    combined_clean_features_diff = np.concatenate(all_clean_features_diff, axis=0)
    combined_adv_features_diff = np.concatenate(all_adv_features_diff, axis=0)

    print(
        f"Combined training data: {len(combined_clean_features_diff)} clean + {len(combined_adv_features_diff)} adversarial samples")
    print(
        f"Combined clean features diff stats - mean: {combined_clean_features_diff.mean():.4f}, std: {combined_clean_features_diff.std():.4f}")
    print(
        f"Combined adv features diff stats - mean: {combined_adv_features_diff.mean():.4f}, std: {combined_adv_features_diff.std():.4f}")

    # 檢查整體分離度
    overall_ratio = combined_adv_features_diff.mean() / (combined_clean_features_diff.mean() + 1e-8)
    print(f"Overall separation ratio: {overall_ratio:.2f}")

    # 訓練統一的 Fuzzy 偵測器 - 使用差異值！
    print("Training unified fuzzy detector on combined FGSM+PGD data...")
    fuzzy_detector = train_fuzzy_detector(
        combined_clean_features_diff,  # 使用差異值
        combined_adv_features_diff,  # 使用差異值
        "MNIST-Optimized-Combined-FGSM-PGD"
    )

    # === 第二階段：在 CIFAR-10 上測試偵測器 ===
    print("\n=== Phase 2: Testing unified fuzzy detector on CIFAR-10 ===")
...
def print_results(seed, detection_results, train_attack_effectiveness, test_attack_effectiveness,
    ...
    """列印最終結果"""
    print("\n" + "=" * 120)
    print("OPTIMIZED CROSS-DATASET UNIFIED FUZZY DETECTOR RESULTS SUMMARY")
    print("=" * 120)
    print(f"Random seed: {seed}")
    print("Training: MNIST + Combined FGSM/PGD → Testing: CIFAR-10 + DeepFool")
    print(f"Method: {CLASS_REFERENCE_SAMPLES}-sample class-based SHAP differences → Unified Fuzzy Detector")
    print()
    ...
    # 偵測結果表格
    header = f"{'Detector':<25} {'Test Attack':<12} {'Dataset':<10} {'Accuracy':<10} {'F1-Score':<10} {'AUC':<8} {'Rules':<6} {'Method':<12}"
    print(header)
    print("-" * len(header))
    ...
    for attack_type in TEST_ATTACK_TYPES:
        if attack_type in detection_results:
            ...
            print(f"{'Optimized-FGSM+PGD-Comb':<25} "
                  f"{attack_type.upper():<12} "
                  f"{'CIFAR-10':<10} "
                  f"{det_results['accuracy']:<10.4f} "
                  f"{det_results['f1']:<10.4f} "
                  f"{det_results['auc']:<8.4f} "
                  f"{det_results.get('num_rules', 'N/A'):<6} "
                  f"{'Fuzzy':<12}")

    print("=" * 120)
    print("Optimization Features:")
    print(f"• Efficient Reference: Uses only {CLASS_REFERENCE_SAMPLES} random same-class samples as reference")
    print("• SHAP Differences: Trains on feature differences, not raw features")
    print("• Class-aware Comparison: Compares against same-class clean samples")
    print("• Reduced Computation: Significantly reduced SHAP computation time")
    print("• Better Separation: Expected higher adversarial/clean ratio")
