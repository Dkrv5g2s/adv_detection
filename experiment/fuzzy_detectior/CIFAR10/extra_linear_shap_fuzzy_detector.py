...
from fuzzy_detectior.CIFAR10.src.shap_signature import generate_linear_shap_signatures
...
def prepare_detector_data(model, clean_data, adv_data, attack_type, device):
    ...
    clean_signatures =generate_linear_shap_signatures(model, clean_images, device)
    ...
    start_time = time.time()
    clean_signatures_noisy = generate_linear_shap_signatures(model, clean_images_noisy, device)
    ...
    start_time = time.time()
    adv_signatures = generate_linear_shap_signatures(model, adv_images, device)
